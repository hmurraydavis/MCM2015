...
availableVaciene = 300 #doses per day (or time step) **varry
...
#        print 'person is: ', person
    ...
    for personNum, person in enumerate(ill):
        if sup>0:
            ...
            break
    for personNum, person in enumerate(two0_inoc):
        if sup>0:
            graph[personNum]['inocFac'].append(1)
            sup=sup-1
        else:
            break
    for personNum, person in enumerate(one0_inoc):
        if sup>0:
            graph[personNum]['inocFac'].append(1)
            sup=sup-1
        else:
            break
    for personNum, person in enumerate(yest_inoc):
        if sup>0:
            graph[personNum]['inocFac'].append(1)
            sup=sup-1
        else:
...
def updateGraph():
    ...
    for day in range(daysToRunModel):
        ...
        vacAvl.append(availableVaciene)
        grabModelDataRT()
        for personNum, personRepDict in enumerate(graph):
...
#    evaluatedValues.append(assesValue)
    ...
    V1 = updateGraph(10)
    demiseWTime=grabModelDataRT()
    B1,Z1,A1 = processBZAdata(demiseWTime)
    dataStore.append(A[-1])
#    evaluatedValues.append(assesValue)
...
#    plt.plot(A,color='r', linewidth=3.0, label = 'More Vaccine')
#    plt.plot(A1, color='b', linewidth=3.0, label = 'More Vaccine'')
#    plt.plot(V0, color='r', linewidth=3.0, label = 'Vaccine Amount, More')
#    plt.plot(V1, color='b', linewidth=3.0, label = 'Vaccine Amount, Less')
#    plt.legend()
#    plt.ylabel('People Living', fontsize=20)
#    plt.xlabel('Time (Days)', fontsize=20)
#    plt.title('Effect of Vaccine Supply on Outcome ', fontsize=30)
#    plt.savefig('VacieneSupplyEffectST.png')
#    plt.show()
    
    fig, ax1 = plt.subplots()
    t = np.arange(0.01, 10.0, 0.01)
    s1 = np.exp(t)
    ax1.plot(A, 'b-', linewidth=3.0,label = 'More Vaccine')
    ax1.plot(A1, 'r-',linewidth=3.0,label = 'Less Vaccine')
    ax1.set_xlabel('Time (Days)', fontsize=20)
    # Make the y-axis label and tick labels match the line color.
    ax1.set_ylabel('People Alive', fontsize=20)#, color='b')
...
#        tl.set_color('b')
    ...
    ax2.plot(V0, 'c')
    ax2.plot(V1, 'm')
    ax2.set_ylabel('Vaccine Supply', fontsize=20)#, color='r')
